...
cp lib"$PORJECT_NAME".dll $RELDIRECTORY
cp "$PORJECT_NAME"_test.exe $RELDIRECTORY
cp $SRCDIR/LITCENSE $RELDIRECTORY
cp $SRCDIR/README.md $RELDIRECTORY
mkdir $RELDIRECTORY/include || true
cp -r $SRCDIR/$PORJECT_NAME $RELDIRECTORY/include
cd $RELDIRECTORY/..
